...
;(function($, window, document, undefined)
{
 ...
 options
	...
	window.notify = function(title, message, options, rotate)
	{
		...
		else
		{
			...
			var classes = ['notification'].concat(settings.classes),
				...
				wrapperOpen = (settings.link.length > 0) ? '<a href="'+settings.link+'"'+altTitle+'>' :'<div'+altTitle+ ' id="' + settings.id + '">',
			...
			if (settings.groupSimilar)
			{
				siblings.each(function(i)
				{
				    ...
				    // Compare messages
					    if (data.title === title && data.message === message) {
	...
	window.notify.defaults = {
		 ...
		 * @var boolean
		 */
		textSeveralSimilars: '%d הודעות דומות',
		 ...
		 * @var boolean
		 */
		textSeveralMore: '%d הודעות נוספות',
