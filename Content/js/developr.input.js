	...
	function _refreshSelectValues()
	{
		...
		if (list.length > 0 && replaced)
		{
			...
			replaced.find('option, optgroup').each(function(i)
			{
				...
				$('<' + node + ((classes.length > 0) ? ' class="' + classes.join(' ') + '"' : '') + ' title="' + text.replace('"', '') + '">' + checkList + text + '</' + node + '>')
					.appendTo(list)
	...
	doc.on('focus', 'span.select, span.selectMultiple', function(event)
	{
		...
		function onBlur(event)
		{
			...
			if (!blurTimeout)
			{
				...
				select.data('selectBlurTimeout', setTimeout(function() { onBlur.call(this, event); }, 1));
				return;
			}
			else
			{
